package riskyken.lighting.common.tileentities;
...
import riskyken.lighting.common.entities.EntityStarLightDespawner;
import riskyken.lighting.utils.Utils;
...
public class TileEntityStarLight extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (startUpTick < ACTIVATE_TICK) {
            ...
            if (startUpTick == 1) {
                /*
                 * PacketHandler.networkWrapper.sendToDimension( new
                 * MessageDimensionSound((byte) 0),
                    ...
                    if (worldObj.blockExists(targetX, targetY, targetZ)) {
                        if (worldObj.isAirBlock(targetX, targetY, targetZ)) {
                            if (worldObj.getBlock(targetX, targetY, targetZ) != ModBlocks.utilitieLight) {
                                if (Utils.getDistance(xCoord, yCoord, zCoord,
                                        targetX, targetY, targetZ) <= radiusCount) {
                                    worldObj.setBlock(targetX, targetY,
                                            targetZ, ModBlocks.utilitieLight,
                                            lightBlockActiveMeta, 2);
                                    blocksPlaced += 1;
                                }
                                if (blocksPlaced > 50) {
                                    return;
                                ...
                                if (worldObj.getBlockMetadata(targetX, targetY,
                                        targetZ) == lightBlockRemoveMeta) {
                                    if (Utils.getDistance(xCoord, yCoord,
                                            zCoord, targetX, targetY, targetZ) <= radiusCount) {
    ...
    public void preRemove() {
        ...
        for (int ix = -type.getLightRadius(); ix <= type.getLightRadius(); ix++) {
            for (int iy = -type.getLightRadius(); iy <= type.getLightRadius(); iy++) {
                for (int iz = -type.getLightRadius(); iz <= type
                        .getLightRadius(); iz++) {
                    ...
                    if (worldObj.getBlock(targetX, targetY, targetZ) == ModBlocks.utilitieLight) {
                        worldObj.setBlockMetadataWithNotify(targetX, targetY,
                                targetZ, lightBlockRemoveMeta, 0);
                    }
        ...
        EntityStarLightDespawner entity = new EntityStarLightDespawner(
                worldObj, xCoord, yCoord, zCoord, type);
    ...
    public Packet getDescriptionPacket() {
        ...
        return new S35PacketUpdateTileEntity(xCoord, yCoord, zCoord, 5,
                compound);
    }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        type = BlockStarLightTypes.getBlockLightType(compound
                .getInteger(TAG_TYPE));
        startUpTick = compound.getShort(TAG_STARTUP_TICK);
