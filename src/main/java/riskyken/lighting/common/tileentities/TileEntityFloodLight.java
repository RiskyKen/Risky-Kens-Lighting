...
import riskyken.lighting.common.entities.EntityFloodLightDespawner;
...
public class TileEntityFloodLight extends TileEntity {
        ...
        if (!worldObj.isAirBlock(targetX, targetY, targetZ)) {
            ...
            if (addingBlocks) {
                if (worldObj.getBlock(targetX, targetY, targetZ) != ModBlocks.utilitieLight) {
                    worldObj.setBlock(targetX, targetY, targetZ,
                            ModBlocks.utilitieLight, facing.ordinal(), 2);
                }
            } else {
                if (worldObj.getBlock(targetX, targetY, targetZ) == ModBlocks.utilitieLight) {
                    if (worldObj.getBlockMetadata(targetX, targetY, targetZ) == facing
                            .ordinal()) {
    ...
    public void preRemove() {
        EntityFloodLightDespawner entity = new EntityFloodLightDespawner(
                worldObj, xCoord, yCoord, zCoord, type, facing);
    ...
    public IIcon getIconForSide(int side, int meta) {
        return ModBlocks.floodLight.getFaceIcon(side, this.facing.ordinal(),
                this.active, this.type.ordinal());
    }
    ...
    public void onDataPacket(NetworkManager net,
        ...
        type = BlockFloodLightTypes.getBlockLightType(compound
                .getInteger(TAG_TYPE));
        worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        markDirty();
    }
