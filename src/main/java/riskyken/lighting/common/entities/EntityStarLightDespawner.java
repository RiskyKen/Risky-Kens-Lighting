...
public class EntityStarLightDespawner extends Entity {
    ...
    public EntityStarLightDespawner(World world, int x, int y, int z,
            BlockStarLightTypes type) {
        super(world);
        this.type = type;
        posX = x;
                        ...
                        if (worldObj.isAirBlock(targetX, targetY, targetZ)) {
                            if (worldObj.getBlock(targetX, targetY, targetZ) == ModBlocks.utilitieLight) {
                                if (worldObj.getBlockMetadata(targetX, targetY,
                                        targetZ) == 14) {
                                    if (Utils.getDistance((int) posX,
                                            (int) posY, (int) posZ, targetX,
                                            targetY, targetZ) >= radiusCount) {
                                        worldObj.setBlockToAir(targetX,
                                                targetY, targetZ);
                                        blocksRemoved += 1;
                                    }
                                    if (blocksRemoved > 50) {
