...
import riskyken.lighting.common.tileentities.TileEntityStarMultiBlock;
import cpw.mods.fml.common.registry.GameRegistry;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class BlockStarMultiBlock extends AbstractModBlock implements
        ITileEntityProvider {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rnd) {
        ...
        int spread = 20;
        for (int i = 0; i < 5; i++) {
            float particleX = (x + 0.5F) + (rnd.nextFloat() * spread) - spread
                    / 2;
            float particleY = (y + 0.5F) + (rnd.nextFloat() * spread) - spread
                    / 2;
            float particleZ = (z + 0.5F) + (rnd.nextFloat() * spread) - spread
                    / 2;

            // EntityStarDust particle = new EntityStarDust(world, particleX,
            // particleY, particleZ, x + 0.5F, y + 0.5F, z + 0.5F, 1F);
